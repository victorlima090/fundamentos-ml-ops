...
_TARGET_COL: str = _CFG_PREP.get("feature_selection", {}).get("target", "isRecommended")
...
_PARQUET_PROCESSADO = _DATA_DIR / "processed" / "wine_quality.parquet"
_PARQUET_FEATURES   = _DATA_DIR / "features"  / "wine_quality_features.parquet"
...
# _IMPUTADOR_AJUSTADO: GroupMedianImputer = _construir_imputador_ajustado()#TODO check if needed
...
def preprocessar_entradas(raw: dict[str, Any]) -> pd.DataFrame:
    ...
    print("Após RatioFeatureTransformer:", df.columns.tolist())
    df = LogTransformer(
        columns=_CFG_PREP.get("log_transform", {}).get("columns", [])
    ).fit_transform(df)
    print("Após LogTransformer:", df.columns.tolist())
    df = FeatureSelector(
    ...
    print("Após reindexação final:", df.columns.tolist())
    ...
    print("Após sanitização de nomes:", df.columns.tolist())
    return df
...
def obter_nomes_colunas_features() -> list[str]:
