...
_NOME_MODELO: str = "best_optuna_model"
...
def carregar_modelo(db_uri: str) -> mlflow.pyfunc.PyFuncModel:
    ...
    """
    print(f"Carregando modelo '{_NOME_MODELO}' do MLflow (SQLite: {db_uri})...")
...
def prever_individual(
    ...
    modelo
        Modelo sklearn carregado via carregar_modelo().
    ...
    """
    resultado = modelo.predict_proba(features_df)
    return float(resultado[0, 1])  # probabilidade da classe 1 (recomendado)
...
def prever_lote(
    ...
    Retorna a probabilidade da classe 1 (recomendado) para cada linha.
    """
    resultado = modelo.predict_proba(features_df)
    return [float(v) for v in resultado[:, 1]]
